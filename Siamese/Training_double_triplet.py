...
#training generator
data_generator = DataGenerator(training_features_file, training_labels_file, offline=True, double=True, training=False)
generator = data_generator.fetch_double_triplet
...
data_generator_val = DataGenerator(val_features_file, val_labels_file, offline=True, double=True)
generator_val = data_generator_val.fetch_double_triplet
...
batch_size = 1
train_data = train_generator.shuffle(data_generator.data_n).batch(1)
...
for epoch in range(epochs):
    loss_avg = []
    for step, (x_batch_train,  x_batch_positive_train,  x_batch_negative_train1, x_batch_negative_train2) in enumerate(train_data):
        ...
        with tf.GradientTape() as tape:
            ...
            negative_a = model(x_batch_negative_train1, training=True)  # Logits for this minibatch
            negative_b = model(x_batch_negative_train2, training=True)  # Logits for this minibatch
            loss = model.doubleTriplet(anchor, positive, negative_a, negative_b, margin=margin)
    ...
    loss_val = []
    for step, (x_batch_val, x_batch_positive_val, x_batch_negative_train1, x_batch_negative_train2) in enumerate(
            val_data):
        ...
        negative_a = model(x_batch_negative_train1, training=False)  # Logits for this minibatch
        negative_b = model(x_batch_negative_train2, training=False)  # Logits for this minibatch
        loss = model.doubleTriplet(anchor, positive, negative_a, negative_b, margin=margin)
