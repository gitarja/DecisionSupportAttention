...
from Utils.PriorFactory import GaussianMixture
...
#optimizer
siamese_optimizer = tf.optimizers.Adamax(lr=lr)
discriminator_optimizer = tf.optimizers.Adamax(lr=lr/5)
generator_optimizer = tf.optimizers.Adamax(lr=lr)
...
for epoch in range(epochs):
    ...
    for images, labels in mini_dataset:
        ...
        samples = GaussianMixture(inner_batch_size, z_dim, n_labels=classes)
        ...
        with tf.GradientTape() as siamese_tape, tf.GradientTape() as discriminator_tape, tf.GradientTape() as generator_tape:
            ...
            #generator loss
            G_loss = binary_loss(z_fake, tf.ones_like(z_true))
